...
interface TemplateExerciseCardProps {
  ...
  index: number;
  isEditing: boolean;
  draft: WorkoutTemplateExercise | null;
  onStartEdit: () => void;
  onCancelEdit: () => void;
  onSaveEdit: () => void;
  onDelete: () => void;
  onDraftChange: (exercise: WorkoutTemplateExercise) => void;
}
...
export default function TemplateExerciseCard({
  ...
  const muscleLabel = (value: WorkoutTemplateExercise) => {
    const muscleGroups = value.muscle_groups ?? [];
    return muscleGroups.length > 0 ? muscleGroups.join(" · ") : "Músculos no especificados";
  };

  return (
    <div className="grid grid-cols-1 items-start gap-3 md:gap-4 lg:grid-cols-[1fr]">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                <label className="flex items-center gap-2 rounded-full border border-border bg-muted/20 px-3 py-2">
                  ...
                  <Input className="h-8 w-14 border-0 bg-transparent p-0 text-center text-sm shadow-none focus-visible:ring-0" type="number" value={draft.sets} onChange={(e) => onDraftChange({ ...draft, sets: Number(e.target.value || 0) })} />
                </label>
                <label className="flex items-center gap-2 rounded-full border border-border bg-muted/20 px-3 py-2">
                  <span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Reps</span>
                  <Input className="h-8 w-14 border-0 bg-transparent p-0 text-center text-sm shadow-none focus-visible:ring-0" type="number" value={draft.reps} onChange={(e) => onDraftChange({ ...draft, reps: Number(e.target.value || 0) })} />
                </label>
                <label className="flex items-center gap-2 rounded-full border border-border bg-muted/20 px-3 py-2">
                  <span className="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Rest</span>
                  <Input className="h-8 w-16 border-0 bg-transparent p-0 text-center text-sm shadow-none focus-visible:ring-0" type="number" value={draft.rest_seconds} onChange={(e) => onDraftChange({ ...draft, rest_seconds: Number(e.target.value || 0) })} />
                </label>

                <div className="flex items-center gap-2 pl-1">
                  <Button variant="outline" size="sm" onClick={onCancelEdit}>Cancelar</Button>
                  <Button size="sm" onClick={onSaveEdit}>Guardar</Button>
                </div>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2 lg:justify-end">
                ...
                <div className="flex items-center gap-1 pl-1">
                  <button type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" onClick={onStartEdit} aria-label="Editar ejercicio">
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button type="button" className="inline-flex h-9 w-9 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-red-500" onClick={onDelete} aria-label="Eliminar ejercicio">
